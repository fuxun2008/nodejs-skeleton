...
function buildViews() {
  ...
  const views = fs.readdirSync(VIEWS_FROM);
...
function processView(view) {
  ...
  // raw content
  const html0 = fs.readFileSync(path.join(VIEWS_FROM, view), 'utf8');
  ...
  // process images
  const html2 = html1.replace(IMAGES_RE, (match, filename, type) => {
  ...
  fs.writeFileSync(path.join(VIEWS_DEST, view), minify(html3, htmlOpts));
}
...
function processScript(filename) {
  ...
  const js1 = js0.replace(IMAGES_RE, (match, file, type) => {
    log(`Found images: ${match}`);
